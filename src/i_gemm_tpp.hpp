...
struct _tile_gemm_64x256 {
  ...
  inline static void test_out(void* C, const size_t ldc, void* acc_pad) {

    auto C_ = reinterpret_cast<int (*)[16][ldc]>(C);
    auto acc_pad_ = reinterpret_cast<int (*)[4][16][16]>(acc_pad);
    ...
    for (int t = 0; t < 4; ++t) {
      ...
      for (int i = 0; i < 16; ++i) {
        ...
        _mm512_storeu_epi32(C_[t][i] + 0, i0);
        _mm512_storeu_epi32(C_[t][i] + 16, i1);
        _mm512_storeu_epi32(C_[t][i] + 32, i2);
        _mm512_storeu_epi32(C_[t][i] + 48, i3);
...
struct _tile_gemm_32x64 {
  ...
  inline static void zero_pad(void* acc_pad) {
    ...
    _tile_stored(TMM0, acc_pad_[0], 64);
    _tile_stored(TMM0, acc_pad_[1], 64);
    _tile_stored(TMM0, acc_pad_[2], 64);
    _tile_stored(TMM0, acc_pad_[3], 64);
  }
  ...
  // use C as scratch pad, not exact output
  inline static void load_acc(void* C) {
    auto C_ = reinterpret_cast<int (*)[16][32]>(C);

    _tile_loadd(TMM0, C_[0][0], 128);
    _tile_loadd(TMM1, C_[0][0] + 16, 128);
    _tile_loadd(TMM2, C_[1][0], 128);
    _tile_loadd(TMM3, C_[1][0] + 16, 128);
  }

  inline static void store(void* C) {
    auto C_ = reinterpret_cast<int (*)[16][32]>(C);
    _tile_stored(TMM0, C_[0][0], 128);
    _tile_stored(TMM1, C_[0][0] + 16, 128);
    _tile_stored(TMM2, C_[1][0], 128);
  ...
  inline static void test_out(void* C, const size_t ldc, void* acc_pad) {
    ...
    auto acc_pad_ = reinterpret_cast<int (*)[16][16]>(acc_pad);
      ...
      #pragma unroll (16)
      for (int i = 0; i < 16; ++i) {
        auto i0 = _mm512_loadu_epi32(acc_pad_[0][i]);
        auto i1 = _mm512_loadu_epi32(acc_pad_[1][i]);
        auto i2 = _mm512_loadu_epi32(acc_pad_[2][i]);
        auto i3 = _mm512_loadu_epi32(acc_pad_[3][i]);

        _mm512_storeu_epi32(C_[i] + 0, i0);
        _mm512_storeu_epi32(C_[i] + 16, i1);
        _mm512_storeu_epi32(C_[i + 16] + 0, i2);
        _mm512_storeu_epi32(C_[i + 16] + 16, i3);
      }
    zero_pad(acc_pad);
  }
};
